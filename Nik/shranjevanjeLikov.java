import java.io.*;
import java.util.*;
public class shranjevanjeLikov {
    public static void main(String[] args) throws IOException {
        //SEMINARSKA NALOGA: Izrisovanje in shranjevanje likov; Program nam omogoca shranjevanje sestih likov v tabelo. Iz tabele lahko nato klicemo poljuben lik. Lik lahko tudi izrisemo v datoteko
    ...
    public static void cmd(Scanner scan) throws IOException{
        ...
        String vsiLiki[] = new String[stLikov]; //Ustvarimo tudi tabelo imenov likov, ki doloca kaksna sorta lika je shranjena na dolocenem mestu
        ...
        while(true){ //dokler uporabnik program ne zeli koncati, se bo stalno izvajala zanka, ki nam omogoca izpis ali vnos lika
            System.out.println("\nKater lik zelite? (0-" + (stLikov-1) + " ali ? ali end)");
            ...
            try{ //celoten program je zaprt v velik try-catch, da se lahko izognemo vsem napakam pri vnosu naenkrat. Takih napak je lahko ogromno, saj imamo veliko vnosov
                if(trenutenLikString.contains("end")) break; //ce je uporabnik napisal end, bomo zapustili zanko in program se bo koncal
                    ...
                    if(input.equals("vnos")){
                        ...
                        vsiLiki[trenutenLik] = scan.next(); //pod pogojem, da zeli uporabnik vnesti lik, ga program vprasa kaksen lik zeli. To shrani v tabelo shranjenih likov
                        ...
                        switch(vsiLiki[trenutenLik]){ //najdemo pravilen lik, razen ce ta ne obstaja. V konstruktor posljemo tudi velikost lika
                            ...
                            case "graf": grafi[trenutenLik] = new graf(size, ""); break; //v graf poleg velikosti posljemo tudi ime datoteke. Ker ne bomo izpisovali v datoteko, je to ime prazno
                        ...
                        switch(vsiLiki[trenutenLik]){ //ce je uporabnik izbral izpis probamo izpisati lik preko metode izpis(). Ta izpise v terminal namesto v datoteko.
                    ...
                    for(int i=0; i<vsiLiki.length; i++){
                        System.out.println(i + " - " + vsiLiki[i]);
                    }
                }
